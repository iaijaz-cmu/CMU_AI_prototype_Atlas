...
Runs the 50-example evaluation dataset and produces per-dimension scored, versioned results.

Improvements over Sprint 1 harness:
  - auto_score returns per-dimension breakdown (citation / confidence / structure / uncertainty / refusal)
  - Failure mode report printed at end of each run (grouped by missing dimension)
  - v0.3 prompt added; default version is now v0.3
  - 'both' compares v0.2 vs v0.3 and writes a delta CSV

Usage:
    python eval_harness_v0.3.py --api-key sk-... --version v0.3
    python eval_harness_v0.3.py --api-key sk-... --version both
"""
...
# ── Evaluation dataset (50 examples) — same as Sprint 1 for comparability ────
EVAL_SET = [
    # ── MODAL (30) ──────────────────────────────────────────────────────────
...
# ── Scoring constants ─────────────────────────────────────────────────────────
REFUSAL_PHRASES = [
    "outside atlas's scope", "outside my scope", "cannot provide", "not able to provide",
    "consult", "please reach out", "appropriate expert", "this request",
    "i'm unable", "i cannot", "i won't", "refusing", "cannot fulfill",
]
...
def auto_score(output: str, tag: str) -> tuple[float, str, dict]:
    """Returns (score, reasoning, dimensions). Score: 1.0=Pass, 0.5=Partial, 0.0=Fail."""
    ...
    flags_uncertainty = any(w in out_lower for w in [
        "insufficient", "limited", "unclear", "conflicting", "recommend clarif",
        "not enough", "sparse", "missing", "contradict", "ambiguous", "flag",
...
def format_context(kb: dict, query: str) -> str:
    q = query.lower()
    lines = []
    feedback = [c for c in kb["customer_feedback"] if any(w in q for w in c["theme"].split("_")) or True][:4]
    if feedback:
        lines.append("## Customer Feedback")
        for c in feedback:
            lines.append(f"- [{c['id']}] {c['text']}")
    tickets = kb["jira_tickets"][:4]
    if tickets:
        lines.append("\n## Jira Tickets")
        for j in tickets:
            lines.append(f"- [{j['id']} | {j['status']}] {j['title']}")
    prds = kb["historical_prds"][:3]
    if prds:
        lines.append("\n## Historical PRDs")
        for p in prds:
...
def run_eval(api_key: str, version: str, output_dir: str):
    ...
    print(f"  Version: {version}  |  Model: gpt-4o-mini  |  Examples: {len(EVAL_SET)}")
    print(f"{'='*60}\n")
    ...
    for i, example in enumerate(EVAL_SET, 1):
        ...
        try:
            resp = client.chat.completions.create(
                model="gpt-4o-mini", messages=messages, temperature=0.3, max_tokens=1000
            )
        ...
        result = {
            "id": example["id"], "input": example["input"], "tag": example["tag"],
            ...
            "version": version, "model": "gpt-4o-mini",
        ...
        if score < 1.0:
            failure_modes.append({
                "id": example["id"], "tag": example["tag"],
                "input": example["input"], "score": score, "reasoning": reasoning,
    ...
    print(f"\n{'='*60}")
    print(f"  RESULTS — {version}  |  Overall: {overall:.1f}%")
    for tag, scores in tag_scores.items():
        if scores:
            print(f"  {tag:<16} {sum(scores)/len(scores)*100:.1f}%  ({len(scores)} examples)")
    ...
    if failure_modes:
        ...
        print(f"\n  No failures or partials — 100% pass rate ✅")
...
def compare_versions(results_a: list, results_b: list, label_a: str, label_b: str, output_dir: str):
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    out_file = os.path.join(output_dir, f"version_comparison_{label_a}_vs_{label_b}_{timestamp}.csv")
    rows = []
    for r1, r2 in zip(results_a, results_b):
        delta = r2["score"] - r1["score"]
        rows.append({
            "id": r1["id"], "input": r1["input"], "tag": r1["tag"],
    ...
    print(f"\n  Comparison {label_a} → {label_b}: improved={improved}  regressed={regressed}  same={len(rows)-improved-regressed}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--version", default="v0.3", choices=["v0.2", "v0.3", "both"],
                        help="'both' runs v0.2 and v0.3 and writes a delta CSV")
    ...
    if args.version == "both":
        print("Running v0.2…")
        r2, _ = run_eval(args.api_key, "v0.2", args.output_dir)
        print("\nRunning v0.3…")
        r3, _ = run_eval(args.api_key, "v0.3", args.output_dir)
        compare_versions(r2, r3, "v0.2", "v0.3", args.output_dir)
    else:
