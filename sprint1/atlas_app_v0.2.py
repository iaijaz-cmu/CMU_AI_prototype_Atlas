...
# ── Config ────────────────────────────────────────────────────────────────────
KNOWLEDGE_BASE_PATH = os.path.join(os.path.dirname(__file__), "knowledge_base_v1.json")
EVAL_LOG_PATH = os.path.join(os.path.dirname(__file__), "eval_log_sprint1.csv")
MODEL = "gpt-4o-mini"
PROMPT_VERSION = "v0.2"
...
user_input = st.text_area("What do you need?", value=prefill, height=100,
...
if run:
    if not api_key:
        ...
        with st.form("eval_form"):
            score = st.select_slider("Score", options=[0.0, 0.5, 1.0],
