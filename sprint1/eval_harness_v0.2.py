...
Runs the 50-example evaluation dataset and produces versioned, scored CSV results.

Usage:
    python eval_harness_v0.2.py --api-key sk-... --version v0.1
    python eval_harness_v0.2.py --api-key sk-... --version v0.2
    python eval_harness_v0.2.py --api-key sk-... --version both
"""
...
def load_kb():
    kb_path = os.path.join(os.path.dirname(__file__), "knowledge_base_v1.json")
...
def format_context(kb: dict, query: str) -> str:
    lines = []
    feedback = kb["customer_feedback"][:3]
    if feedback:
        lines.append("## Customer Feedback")
        for c in feedback:
            lines.append(f"- [{c['id']}] {c['text']}")
    tickets = kb["jira_tickets"][:3]
    if tickets:
        lines.append("\n## Jira Tickets")
        for j in tickets:
...
def run_eval(api_key: str, version: str, output_dir: str):
    ...
    print(f"\n{'='*60}")
    print(f"  Atlas Evaluation Harness — Sprint 1")
    ...
    for i, example in enumerate(EVAL_SET, 1):
        ...
        try:
            resp = client.chat.completions.create(
                model="gpt-4o-mini", messages=messages, temperature=0.3, max_tokens=800
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Atlas Evaluation Harness — Sprint 1")
    ...
    if args.version == "both":
        run_eval(args.api_key, "v0.1", args.output_dir)
        run_eval(args.api_key, "v0.2", args.output_dir)
    else:
